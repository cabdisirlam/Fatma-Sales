...
function initializeSystemSheets(ss) {
  ...
  const v3Schema = {
    ...
    [CONFIG.SHEETS.CUSTOMERS]: ['Customer_ID', 'Customer_Name', 'Phone', 'Email', 'Location', 'KRA_PIN', 'Customer_Type', 'Credit_Limit', 'Current_Balance', 'Opening_Balance', 'Total_Paid', 'Total_Purchases', 'Last_Purchase_Date', 'Loyalty_Points', 'Advance_Credit', 'Status', 'Created_Date', 'Created_By'],
    ...
    [CONFIG.SHEETS.QUOTATIONS]: ['Quotation_ID', 'DateTime', 'Customer_ID', 'Customer_Name', 'Item_ID', 'Item_Name', 'Qty', 'Batch_ID', 'Unit_Price', 'Line_Total', 'Subtotal', 'Delivery_Charge', 'Discount', 'Grand_Total', 'Created_By', 'Location', 'KRA_PIN', 'Status', 'Valid_Until', 'Converted_Sale_ID'],
...
/**
 * Ensure Customers header row matches expected order (includes Advance_Credit).
 * This avoids misalignment when opening balances/advance credit are used.
 */
function harmonizeCustomerHeaders(ss) {
  try {
    const sheet = ss.getSheetByName(CONFIG.SHEETS.CUSTOMERS);
    if (!sheet) return;

    const targetHeaders = ['Customer_ID', 'Customer_Name', 'Phone', 'Email', 'Location', 'KRA_PIN', 'Customer_Type', 'Credit_Limit', 'Current_Balance', 'Opening_Balance', 'Total_Paid', 'Total_Purchases', 'Last_Purchase_Date', 'Loyalty_Points', 'Advance_Credit', 'Status', 'Created_Date', 'Created_By'];
    const headerRange = sheet.getRange(1, 1, 1, targetHeaders.length);
    headerRange.setValues([targetHeaders]);
    formatHeader(sheet, headerRange);
    Logger.log('バ" Harmonized Customer headers for alignment.');
  } catch (error) {
    Logger.log('Error harmonizing customer headers: ' + error.message);
  }
}
